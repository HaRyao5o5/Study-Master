...
interface ExportObject {
  meta: {
    type: string; // 'backup' | 'course' | 'quiz'
    version: string;
    createdAt: string;
  };
  data: any;
...
// ■ エクスポート（保存）機能
export const exportToFile = (data: any, type: string, fileNamePrefix: string): void => {
...
// ■ インポート（読込）機能
export const importFromFile = (file: File | null, expectedType: string, callback: (data: any) => void): void => {
  if (!file) return;
    ...
    try {
          ...
          if (expectedType === 'course' && json.title && json.quizzes !== undefined) {
            ...
            if (confirm("古い形式のコースファイルです。読み込みますか？")) {
              ...
              const processedData: Course = {
                ...
                quizzes: (json.quizzes || []).map((q: any): Quiz => ({
                  ...q,
                  id: `quiz-${generateId()}`
                }))
              };
      ...
      if (expectedType === 'course') {
        processedData = {
          ...processedData,
          id: `course-${generateId()}`,
          quizzes: (processedData.quizzes || []).map((q: any) => ({
            ...q,
            id: `quiz-${generateId()}`
          }))
        };
