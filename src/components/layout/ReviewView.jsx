// src/components/layout/ReviewView.jsx
import React, { useState, useMemo } from 'react';
import { ArrowLeft, RefreshCw, Award, TrendingUp } from 'lucide-react';
import { useNavigate } from 'react-router-dom';

const ReviewView = ({ wrongHistory, masteredQuestions, courses, onBack }) => {
  ...
  const reviewItems = useMemo(() => {
    ...
    wrongHistory.forEach((quizId) => {
      ...
      Object.values(masteredQuestions).forEach(courseQuizzes => {
        if (courseQuizzes[quizId]) {
          isMastered = true;
        }
      });

      if (isMastered) return; // マスター済みならスキップ
  ...
  const handleStartReview = (courseId, quizId) => {
