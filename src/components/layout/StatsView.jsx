// src/components/layout/StatsView.jsx
import React, { useMemo, useState } from 'react'; // useState追加
import { Trophy, Flame, Star, AlertTriangle, ArrowLeft, Medal, Lock, Zap, X } from 'lucide-react'; // X追加
import { getLevelInfo, TITLES } from '../../utils/gamification';

const StatsView = ({ userStats, errorStats, courses, onBack }) => {
  ...
  const [selectedTitle, setSelectedTitle] = useState(null); // 詳細表示用の状態
  ...
  const topWeaknesses = useMemo(() => {
    ...
    return sortedErrors.map(([id, count]) => {
      ...
      return {
        id,
        count,
        text: question ? question.text : '（削除された問題）'
      };
    });
  }, [errorStats, courses]);
