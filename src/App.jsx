...
import { Folder, FileText, ChevronRight, Play, Settings, Clock, CheckCircle, XCircle, RotateCcw, Home, ArrowLeft, Layers, Brain, Target, Trash2, Lock, Shuffle, Moon, Sun } from 'lucide-react';

// ==========================================
// DATA SECTION
// ==========================================
// プレビュー環境での動作を保証するため、データを直接定義します。
const RAW_DATA = [
  {
    "test_name": "第1回 小テスト",
    "questions": [
      ...
      },
      {
        ...
        "options": { "ア": "1.2", "イ": "6.4", "ウ": "8.0", "エ": "9.6" },
        "answer": "エ"
      },
      {
        "question_number": 4,
        ...
        "options": { "ア": "50", "イ": "100", "ウ": "150", "エ": "250" },
        "answer": "ア"
      }
    ]
  },
    ...
    "questions": [
      {
        ...
        "options": { "ア": "セッション層", "イ": "データリンク層", "ウ": "トランスポート層", "エ": "ネットワーク層" },
        "answer": "エ"
      },
      {
        "question_number": 2,
        "question_text": "OSI基本参照モデルの各層で中継する装置を、物理層で中継する装置、データリンク層で中継する装置、ネットワーク層で中継する装置の順に並べたものはどれか。(R3)",
        "options": { "ア": "ブリッジ, リピータ, ルータ", "イ": "ブリッジ, ルータ, リピータ", "ウ": "リピータ, ブリッジ, ルータ", "エ": "リピータ, ルータ, ブリッジ" },
        "answer": "ウ"
      },
      {
        "question_number": 3,
        "question_text": "イーサネットで使用されるメディアアクセス制御方式であるCSMA/CDに関する記述として、適切なものはどれか。(R2秋)",
        "options": { "ア": "それぞれのステーションがキャリア検知を行うとともに、送信データの衝突が起きた場合は再送する。", "イ": "タイムスロットと呼ばれる単位で分割して、同一周波数において複数の通信を可能にする。", "ウ": "データ送受信の開始時にデータ送受信のネゴシエーションとしてRTS/CTS方式を用い、受信の確認はACKを使用する。", "エ": "伝送路上にトークンを巡回させ、トークンを受け取った端末だけがデータ送信できる。" },
        "answer": "ア"
      },
      {
        "question_number": 4,
        "question_text": "ルータの機能に関する記述のうち、適切なものはどれか。(R4秋)",
        "options": { "ア": "MACアドレステーブルの登録情報によって、データフレームをあるポートだけに中継するか、全てのポートに中継するか判断する。", "イ": "OSI基本参照モデルのデータリンク層において、ネットワーク同士を接続する。", "ウ": "OSI基本参照モデルのトランスポート層からアプリケーション層までの階層で、プロトコルの変換を行う。", "エ": "伝送媒体やアクセス制御方式の異なるネットワークの接続が可能であり、送信データのIPアドレスを識別し、データの転送経路を決定する。" },
        "answer": "エ"
      }
    ]
  },
    ...
    "questions": [
      {
        ...
        "options": { "ア": "アプリケーション層", "イ": "インターネット層", "ウ": "トランスポート層", "エ": "リンク層" },
        "answer": "ウ"
      },
      {
        "question_number": 2,
        "question_text": "次のネットワークアドレスとサブネットマスクをもつネットワークがある。このネットワークをあるPCが利用する場合、そのPCに割り振ってはいけないIPアドレスはどれか。\nネットワークアドレス: 200.170.70.16\nサブネットマスク: 255.255.255.240",
        "options": { "ア": "200.170.70.17", "イ": "200.170.70.20", "ウ": "200.170.70.30", "エ": "200.170.70.31" },
        "answer": "エ"
      },
      {
        "question_number": 3,
        "question_text": "クライアントAがポート番号8080のHTTPプロキシサーバBを経由してポート番号80のWebサーバCにアクセスしているとき、宛先ポート番号が常に8080になるTCPパケットはどれか。(R1秋)",
        "options": { "ア": "AからBへのHTTP要求及びCからBへのHTTP応答。", "イ": "AからBへのHTTP要求だけ。", "ウ": "BからAへのHTTP要求だけ。", "エ": "BからCへのHTTP要求及びCからBへのHTTP応答。" },
        "answer": "イ"
      },
      {
        "question_number": 4,
        "question_text": "DHCPの説明として、適切なものはどれか。(R3)",
        "options": { "ア": "IPアドレスの設定を自動化するためのプロトコルである。", "イ": "ディレクトリサービスにアクセスするためのプロトコルである。", "ウ": "電子メールを転送するためのプロトコルである。", "エ": "プライベートIPアドレスをグローバルIPアドレスに変換するためのプロトコルである。" },
        "answer": "ア"
      }
    ]
  },
    ...
    "questions": [
      {
        ...
        "options": { "ア": "arp", "イ": "ipconfig", "ウ": "netstat", "エ": "ping" },
        "answer": "ア"
      },
      {
        "question_number": 2,
        ...
        "options": { "ア": "管理ステーションから定期的にネットワーク機器のMIB (Management Information Base) 情報を取得して, 稼働監視や性能管理を行うためのネットワーク管理手法", "イ": "データ転送機能をもつネットワーク機器同士が経路情報を交換して, ネットワーク全体のデータ転送経路を決定する方式", "ウ": "ネットワーク制御機能とデータ転送機能を実装したソフトウェアを, 仮想環境で利用するための技術", "エ": "ネットワーク制御機能とデータ転送機能を論理的に分離し, コントローラと呼ばれるソフトウェアで, データ転送機能をもつネットワーク機器の集中制御を可能とするアーキテクチャ" },
        "answer": "エ"
      },
      {
        "question_number": 3,
        "question_text": "SMTPの説明として, 適切なものはどれか。(H17秋)",
        "options": { "ア": "Webサーバに格納されている情報をアクセスするためのプロトコルである。", "イ": "電子化された文字, 図形, イメージが混在した文書の作成や編集を行うシステムである。", "ウ": "電子メールを転送するためのプロトコルである。", "エ": "文書の構造表現が可能な文書記述用言語の一つである。" },
        "answer": "ウ"
      },
      {
        "question_number": 4,
        ...
        "options": { "ア": "CGI", "イ": "HTML", "ウ": "MIME", "エ": "URL" },
        "answer": "ア"
      }
    ]
  },
    ...
    "questions": [
      {
        ...
        "options": { "ア": "Webサイトへの不正な入力を排除するために, Webサイトの入力フォームの入力データから, HTMLタグ, JavaScript, SQL文などを検出し, それらを他の文字列に置き換えるプログラム", "イ": "サーバへの侵入口となる脆弱なポートを探すために, 攻撃者のPCからサーバのTCPポートに順にアクセスするプログラム", "ウ": "利用者の意図に反してPCにインストールされ, 利用者の個人情報やアクセス履歴などの情報を収集するプログラム", "エ": "利用者のパスワードを調べるために, サーバにアクセスし, 辞書に載っている単語を総当たりで試すプログラム" },
        "answer": "ウ"
      },
      {
        "question_number": 2,
        "question_text": "マルウェアについて, トロイの木馬とワームを比較したとき, ワームの特徴はどれか。(H29秋)",
        "options": { "ア": "勝手にファイルを暗号化して正常に読めなくする", "イ": "単独のプログラムとして不正な動作を行う", "ウ": "特定の条件になるまで活動できずに待機する", "エ": "ネットワークやリムーバブルメディアを媒介として自ら感染を広げる" },
        "answer": "エ"
      },
      {
        "question_number": 3,
        "question_text": "SQLインジェクション攻撃の説明として, 適切なものはどれか。(R2)",
        "options": { "ア": "Webアプリケーションのデータ操作言語の呼び出し方に不備がある場合に, 攻撃者が悪意をもって構成した文字列を入力することによって, データベースのデータの不正な取得, 改ざん及び削除をする攻撃", "イ": "Webサイトに対して, 他のサイトを介して大量のパケットを送り付け, そのネットワークトラフィックの異常を高めてサービスを提供不能にする攻撃", "ウ": "確保されているメモリ空間の下限又は上限を超えてデータの書き込みと読み出しを行うことによって, プログラムを異常終了させたりデータエリアに挿入された不正なコードを実行させたりする攻撃", "エ": "攻撃者が罠を仕掛けたWebページを利用者が閲覧し, 当該ページ内のリンクをクリックしたときに, 不正スクリプトを含む文字列が脆弱なWebサーバに送り込まれ, レスポンスに埋め込まれた不正スクリプトの実行によって, 情報漏洩をもたらす攻撃" },
        "answer": "ア"
      },
      {
        "question_number": 4,
        ...
        "options": { "ア": "A社WebサーバにアクセスしようとするA社従業員", "イ": "A社WebサーバにアクセスしようとするB社従業員", "ウ": "B社WebサーバにアクセスしようとするA社従業員", "エ": "B社WebサーバにアクセスしようとするB社従業員" },
        "answer": "イ"
      }
    ]
  },
    ...
    "questions": [
      {
        ...
        "options": { "ア": "インターネットから受け取ったパケットに改ざんがある場合は修正し, 改ざんが修正できない場合には, ログを取って内部ネットワークへの通過を阻止する", "イ": "インターネットから受け取ったパケットのヘッダー部分及びデータ部分に, 改ざんがあるかどうかをチェックし, 改ざんがあった場合にはそのパケットを除去する", "ウ": "動的に割り当てられたTCPポート番号をもったパケットを, 受信側で固定値のTCPポート番号をもったパケットに変更して, 内部のネットワークへの通過を許可する", "エ": "特定のTCPポート番号をもったパケットだけに, インターネットから内部ネットワークへの通過を許可する" },
        "answer": "エ"
      },
      {
        "question_number": 2,
        "question_text": "WAFの説明はどれか。(H28秋)",
        "options": { "ア": "Webアクセスに対するアクセス内容を監視し, 攻撃とみなされるパターンを検知したときに当該するアクセスを遮断する", "イ": "Wi-Fiアライアンスが認定した無線LANの暗号化方式の規格であり, AES暗号に対応している", "ウ": "様々なシステムの動作ログを一元的に蓄積, 管理し, セキュリティ上の脅威となる事象をいち早く検知, 分析する", "エ": "ファイアウォール機能を有し, ウイルス対策, 侵入検知などを連携させ, 複数のセキュリティ機能を統合的に管理する" },
        "answer": "ア"
      },
      {
        "question_number": 3,
        "question_text": "公開鍵暗号方式の用法に関する記述のうち, 送信者が間違いなく本人であることを受信者が確認できるのはどれか。(H17春)",
        "options": { "ア": "送信者は自分の公開鍵で暗号化し, 受信者は自分の秘密鍵で復号する", "イ": "送信者は自分の秘密鍵で暗号化し, 受信者は送信者の公開鍵で復号する", "ウ": "送信者は受信者の公開鍵で暗号化し, 受信者は自分の秘密鍵で復号する", "エ": "送信者は受信者の秘密鍵で暗号化し, 受信者は自分の公開鍵で復号する" },
        "answer": "イ"
      },
      {
        "question_number": 4,
        "question_text": "メッセージにRSA方式のデジタル署名を付与して2者間で送受信する。そのときのデジタル署名の検証鍵と使用方法はどれか。(R1秋)",
        "options": { "ア": "受信者の公開鍵であり, 送信者がメッセージダイジェストからデジタル署名を作成する際に使用する", "イ": "受信者の秘密鍵であり, 受信者がデジタル署名からメッセージダイジェストを算出する際に使用する", "ウ": "送信者の公開鍵であり, 受信者がデジタル署名からメッセージダイジェストを算出する際に使用する", "エ": "送信者の秘密鍵であり, 送信者がメッセージダイジェストからデジタル署名を作成する際に使用する" },
        "answer": "ウ"
      }
    ]
  },
    ...
    "questions": [
      {
        ...
        "options": { "ア": "リスク特定, リスク評価, リスク受容", "イ": "リスク特定, リスク分析, リスク評価", "ウ": "リスク分析, リスク対応, リスク受容", "エ": "リスク分析, リスク評価, リスク対応" },
        "answer": "イ"
      },
      {
        "question_number": 2,
        "question_text": "リスク対応のうち, リスクファイナンシングに該当するものはどれか。(H31春)",
        "options": { "ア": "システムが被害を受けるリスクを想定して, 保険を掛ける。", "イ": "システムの被害につながるリスクの顕在化を抑える対策に資金を投入する。", "ウ": "リスクが大きいと評価されたシステムを廃止し, 新たなセキュアなシステムの構築に資金を投入する。", "エ": "リスクが顕在化した場合のシステムの被害を小さくする設備に資金を投入する。" },
        "answer": "ア"
      },
      {
        "question_number": 3,
        "question_text": "ペネトレーションテストに該当するものはどれか。(R6)",
        "options": { "ア": "検査対象の実行プログラムの設計書, ソースコードに着目し, 開発プロセスの各程にセキュリティ上の問題がないかどうかをツールや目視で確認する。", "イ": "公開Webサーバの各コンテンツファイルのハッシュ値を管理し, 定期的に各ファイルから生成したハッシュ値と一致するかどうかを確認する。", "ウ": "公開Webサーバや組織のネットワークの脆弱性を探索し, サーバに実際に侵入できるかどうかを確認する。", "エ": "内部ネットワークのサーバやネットワーク機器のIPFIX情報から, 各PCの通信に異常な振る舞いがないかどうかを確認する。" },
        "answer": "ウ"
      },
      {
        "question_number": 4,
        "question_text": "ファジングで得られるセキュリティ上の効果はどれか。(H31春)",
        "options": { "ア": "ソフトウェアの脆弱性を自動的に修正できる。", "イ": "ソフトウェアの脆弱性を検出できる。", "ウ": "複数のログデータを相関分析し, 不正アクセスを検知できる。", "エ": "利用者IDを統合的に管理し, 統一したパスワードポリシーを適用できる。" },
        "answer": "イ"
      }
    ]
  },
    ...
    "questions": [
      {
        ...
        "options": { "ア": "あらかじめ設定した運用基準に従って, メールサーバを通過する送受信メールをフィルタリングすること", "イ": "外部からの攻撃や不正なアクセスからサーバを防御すること", "ウ": "磁気ディスクなどの書き換え可能な記憶媒体を単に初期化するだけではデータを復元できる可能性があるので, 任意のデータ列で上書きすること", "エ": "不正アクセスなどコンピュータに関する犯罪の法的な証拠性を確保できるように, 原因究明に必要な情報を保全, 収集, 分析をすること" },
        "answer": "エ"
      },
      {
        "question_number": 2,
        "question_text": "HTTPS (HTTP over SSL/TLS) の機能を用いて実現できるものはどれか。(H26秋)",
        "options": { "ア": "SQLインジェクションによるWebサーバへの攻撃を防ぐ。", "イ": "TCPポート80番と443番以外の通信を遮断する。", "ウ": "Webサーバとブラウザの間の通信を暗号化する。", "エ": "Webサーバへの不正なアクセスをネットワーク層でのパケットフィルタリングによって制限する。" },
        "answer": "ウ"
      },
      {
        "question_number": 3,
        ...
        "options": { "ア": "PCとDBサーバ間の通信データを暗号化するとともに, 正当なDBサーバであるかを検証することができるようになる。", "イ": "PCとDBサーバ間の通信データを暗号化するとともに, 利用者を認証することができるようになる。", "ウ": "PCとWebサーバ間の通信データを暗号化するとともに, 正当なDBサーバであるかを検証することができるようになる。", "エ": "PCとWebサーバ間の通信データを暗号化するとともに, 利用者を認証することができるようになる。" },
        "answer": "エ"
      },
      {
        "question_number": 4,
        "question_text": "電子メールに用いられるS/MIMEの機能はどれか。(H20春)",
        "options": { "ア": "内容の圧縮", "イ": "内容の暗号化と署名", "ウ": "内容の開封通知", "エ": "内容の再送" },
        "answer": "イ"
      }
    ]
  },
    ...
    "questions": [
      {
        ...
        "options": { "ア": "関係は, 表に対応付けられる", "イ": "属性も列も, 左から右に順序付けられる", "ウ": "タプルも行も, ともに重複しない", "エ": "定義域は, 文字型又は文字列型に対応付けられる" },
        "answer": "ア"
      },
      {
        "question_number": 2,
        "question_text": "DBMSにおいて, スキーマを決める機能はどれか。(H27秋)",
        "options": { "ア": "機密保護機能", "イ": "障害回復機能", "ウ": "定義機能", "エ": "保全機能" },
        "answer": "ウ"
      },
      {
        "question_number": 3,
        "question_text": "DBMSが, 3層スキーマアーキテクチャを採用する目的として, 適切なものはどれか。(H27春)",
        "options": { "ア": "関係演算によって元の表から新たな表を導出し, それが実在しているように見せる。", "イ": "対話的に使われるSQL文を, アプリケーションプログラムからも使えるようにする。", "ウ": "データの物理的な格納構造を変更しても, アプリケーションプログラムに影響が及ばないようにする。", "エ": "プログラム言語を限定して, アプリケーションプログラムとDBMSを緊密に結合する。" },
        "answer": "ウ"
      },
      {
        "question_number": 4,
        "question_text": "データベースを記録媒体にどのように格納するかを記述したものはどれか。(H18秋)",
        "options": { "ア": "概念スキーマ", "イ": "外部スキーマ", "ウ": "サブスキーマ", "エ": "内部スキーマ" },
        "answer": "エ"
      }
    ]
  },
    ...
    "questions": [
      {
        ...
        "options": { "ア": "E-Rモデル", "イ": "階層モデル", "ウ": "関係モデル", "エ": "ネットワークモデル" },
        "answer": "イ"
      },
      {
        "question_number": 2,
        "question_text": "関係データモデルにおいて, 属性が取り得る値の集合を意味する用語はどれか。(H18秋)",
        "options": { "ア": "関係 (リレーション)", "イ": "実現値", "ウ": "タプル (組)", "エ": "定義域" },
        "answer": "エ"
      },
      {
        "question_number": 3,
      ...
      },
      {
        ...
        "options": { "ア": "関係データベースの表として実装することを前提に表現する", "イ": "業務で扱う情報をエンティティ及びエンティティ間のリレーションシップとして表現する", "ウ": "データの生成から消滅に至るデータ操作を表現する", "エ": "リレーションシップは, 業務上の手順を表現する" },
        "answer": "イ"
      }
    ]
  },
    ...
    "questions": [
      {
        ...
        "options": { "ア": "キー制約", "イ": "検査制約", "ウ": "参照制約", "エ": "表明" },
        "answer": "ウ"
      },
      {
        "question_number": 2,
        "question_text": "関係データベースにおいて, 外部キーを定義する目的として, 適切なものはどれか。(H28春)",
        "options": { "ア": "関連する相互のテーブルにおいて, レコード間の参照一貫性が維持される制約をもたせる。", "イ": "関連する相互テーブルの格納場所を近くに配置することによって, 検索, 更新を高速に行う。", "ウ": "障害によって破壊されたレコードを, テーブル間の相互の関係から可能な限り復旧させる。", "エ": "レコードの削除, 追加の繰返しによる, レコードの格納エリアのフラグメンテーションを防止する。" },
        "answer": "ア"
      },
      {
        "question_number": 3,
        "question_text": "関係データベースの主キー制約の条件として, キー値が重複していないことの他に, 主キーを構成する列に必要な条件はどれか。(H25秋)",
        "options": { "ア": "キー値が空でないこと", "イ": "構成する列が1つであること", "ウ": "表の先頭に定義されている列であること", "エ": "別の表の候補キーとキー値が一致していること" },
        "answer": "ア"
      },
      {
        "question_number": 4,
        "question_text": "関係データベースの主キーの性質として, 適切なものはどれか。(H21秋)",
        "options": { "ア": "主キーとした列に対して検索条件を指定しなければ, 行の検索はできない。", "イ": "数値型の列を主キーに指定すると, その列は算術演算の対象としては使えない。", "ウ": "1つの表の中に, 主キーの値が同じ行が複数存在することはできない。", "エ": "複数の列からなる主キーを構成することはできない。" },
        "answer": "ウ"
      }
    ]
  },
    ...
    "questions": [
      {
        ...
        "options": { "ア": "結合 (Join)", "イ": "射影 (Projection)", "ウ": "選択 (Selection)", "エ": "和 (Union)" },
        "answer": "イ"
      },
      {
        "question_number": 2,
        "question_text": "関係代数の演算のうち, 関係 R, S の直積 R × S に対応する SELECT 文はどれか。ここで, 関係 R, S を表 R, S に対応させ, 表 R 及び S にそれぞれ行の重複はないものとする。(H28秋)",
        "options": { "ア": "SELECT * FROM R, S", "イ": "SELECT * FROM R EXCEPT SELECT * FROM S", "ウ": "SELECT * FROM R UNION SELECT * FROM S", "エ": "SELECT * FROM R INTERSECT SELECT * FROM S" },
        "answer": "ア"
      },
      {
        "question_number": 3,
        "question_text": "列 A1-A5 から成る R 表に対する次の SQL 文は, 関係代数のどの演算に対応するか。(H25春)\nSELECT A1, A2, A3 FROM R WHERE A4='a'",
        "options": { "ア": "結合と射影", "イ": "差と選択", "ウ": "選択と射影", "エ": "和と射影" },
        "answer": "ウ"
      },
      {
        "question_number": 4,
        ...
        "options": { "ア": "R ∩ S", "イ": "R ∪ S", "ウ": "R × S", "エ": "R ÷ S" },
        "answer": "ア"
      }
    ]
  },
    ...
    "questions": [
      {
        ...
        "options": { "ア": "発注 (注文番号, 注文数量), 商品 (商品番号, 商品名)", "イ": "発注 (注文番号, 注文数量), 商品 (注文番号, 商品番号, 商品名)", "ウ": "発注 (注文番号, 商品番号, 注文数量), 商品 (商品番号, 商品名)", "エ": "発注 (注文番号, 商品番号, 注文数量), 商品 (商品番号, 商品名, 注文番号)" },
        "answer": "ウ"
      },
      {
        "question_number": 2,
        ...
        "options": { "ア": "注文明細 (注文番号, 顧客番号, 商品番号, 数量, 販売単価)", "イ": "注文明細 (注文番号, 顧客番号, 数量, 販売単価)", "ウ": "注文明細 (注文番号, 商品番号, 数量, 販売単価)", "エ": "注文明細 (注文番号, 数量, 販売単価)" },
        "answer": "ウ"
      },
      {
        "question_number": 3,
        "question_text": "関係を第三正規形まで正規化して設計する目的はどれか。(H26秋)",
        "options": { "ア": "値の重複をなくすことによって, 格納効率を向上させる。", "イ": "関係を細かく分解することによって, 整合性制約を排除する。", "ウ": "冗長性を排除することによって, 更新時異状を回避する。", "エ": "属性間の結合度を低下させることによって, 更新時のロック待ちを減らす。" },
        "answer": "ウ"
      },
      {
        "question_number": 4,
        ...
        "options": { "ア": "第二正規形", "イ": "第三正規形", "ウ": "第四正規形", "エ": "非正規形" },
        "answer": "ア"
      }
    ]
  }
...
const generateRealData = () => {
  ...
  if (!RAW_DATA) return []; // データがない場合のガード

  const quizzes = RAW_DATA.map((item, index) => {
    const quizId = `quiz-${index + 1}`;
    const questions = item.questions.map((q, qIndex) => {
      const options = [q.options['ア'], q.options['イ'], q.options['ウ'], q.options['エ']];
      const correctMap = { 'ア': 0, 'イ': 1, 'ウ': 2, 'エ': 3 };
      return {
        id: `q${index + 1}-${q.question_number}`,
        text: q.question_text,
        options: options,
        correctIndex: correctMap[q.answer],
    ...
    return {
      ...
      description: `全${questions.length}問`,
      questions: questions
    };
  });

  return [{ id: courseId, title: '情報処理入門２', description: '2025年度 後期', quizzes: quizzes }];
};
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
    {courses.map(course => (
      <div 
        ...
        className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm hover:shadow-md border border-gray-100 dark:border-gray-700 cursor-pointer transition-all hover:border-blue-300 dark:hover:border-blue-500 flex flex-col items-center justify-center h-48 group"
      >
        ...
        <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100 text-center">{course.title}</h3>
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">{course.description}</p>
...
// 3. Quiz List
const QuizListView = ({ course, onSelectQuiz, wrongHistory, onSelectReview }) => {
    ...
    <div className="space-y-8">
              ...
              <div className="flex items-center bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded px-2 py-1">
                <input
                  ...
                  onChange={(e) => {
                     ...
                     if (!isNaN(val) && val >= 1) setMockCount(val > totalQ ? totalQ : val);
                  }}
                  className="w-10 text-center font-bold text-gray-700 dark:text-gray-200 bg-transparent outline-none"
                />
      ...
      {wrongHistory.length > 0 && (
        <div className="bg-red-50 dark:bg-red-900/20 rounded-xl p-6 shadow-sm border border-red-100 dark:border-red-900/50">
          ...
          <p className="text-sm text-red-600 dark:text-red-400 mb-4">過去に間違えた問題が蓄積されています。</p>
          <button 
      ...
      {/* リスト */}
      <div>
        <h3 className="text-sm font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3">各回小テスト</h3>
        <div className="grid grid-cols-1 gap-3">
          {course.quizzes.map(quiz => (
            <div 
              key={quiz.id}
              onClick={() => onSelectQuiz(quiz)}
              className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 hover:border-blue-400 dark:hover:border-blue-500 cursor-pointer flex items-center justify-between transition-all"
            >
              <div className="flex items-center">
                <div className="bg-yellow-100 dark:bg-yellow-900/50 p-3 rounded-lg mr-4 text-yellow-600 dark:text-yellow-400">
                  <FileText size={24} />
                </div>
          ...
          <div className="mb-8">
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
              {quiz.questions.map((q, idx) => (
                <div key={q.id} className="text-sm p-3 bg-gray-50 dark:bg-gray-700/50 rounded border border-gray-100 dark:border-gray-600 text-gray-600 dark:text-gray-300">
    ...
    if (shuffleOptions) {
      order = order.map(q => {
        ...
        return {
          ...
          correctIndex: shuffledOptions.findIndex(o => o.originalIndex === q.correctIndex),
        };
      });
    }
    return order;
...
// 7. Settings View (New)
const SettingsView = ({ theme, toggleTheme, onBack }) => {
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden max-w-2xl mx-auto">
      ...
      <div className="p-6 space-y-6">
        {/* テーマ設定 */}
        <div className="flex items-center justify-between p-4 border rounded-xl border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50">
          <div className="flex items-center">
            <div className="p-3 rounded-full bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 mr-4">
              {theme === 'dark' ? <Moon size={24} /> : <Sun size={24} />}
            </div>
            <div>
              <h3 className="font-bold text-gray-800 dark:text-gray-100">ダークモード</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {theme === 'dark' ? '目に優しいダークテーマを使用中' : '明るいライトテーマを使用中'}
              </p>
            </div>
          </div>
...
export default function App() {
  const [view, setView] = useState('home'); // home, course, quiz_menu, quiz_play, result, settings
  ...
  const [theme, setTheme] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('study-master-theme') || 'light';
    }
    return 'light';
  });
  ...
  const goToSettings = () => {
    // 現在のビューを保存しておくロジックもアリだが、今回はシンプルに
    setView('settings');
  };
  ...
  const getPath = () => {
    ...
    if (selectedQuiz && view !== 'course') path.push({ title: selectedQuiz.title, id: selectedQuiz.id, type: 'quiz_menu' });
    return path;
  };
      ...
      <header className="bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700 sticky top-0 z-10 transition-colors">
        <div className="max-w-4xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <div className="text-xs font-medium text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded hidden sm:block">
              Test Prep Mode
            </div>
            <button 
